...
boolean canGetExactChange(int targetMoney, int[] denominations) {
    ...
    int pos = denominations.length; //start from last element
    
    return (change(targetMoney, denominations, pos -1)) == 0;
  }

  int change(int targetMoney, int[] denominations, int pos) {
    if(pos < 0 || targetMoney == 0) {
        return targetMoney;
    }
    
    int remainder = targetMoney - (((int)targetMoney / denominations[pos]) * denominations[pos]);
    return change(remainder, denominations, pos - 1);
  }
