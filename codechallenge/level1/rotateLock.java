...
Sample Explanation
In the first case, there are 33 integers on the lock, and the sequence of integers to be selected is [1,2,3][1,2,3]. One optimal way to enter the code is: select 11 →→ rotate to 22 (11 second) →→ select 22 →→ rotate to 33 (11 second) →→ select 33. The total time taken is 1+1=21+1=2 seconds.
In the second case, the lock consists of the integers 11 through 1010, and the sequence to be selected is [9,4,4,8][9,4,4,8]. One optimal way to enter the code is: rotate from 11 backwards to 99 (22 seconds) →→ select 99 →→ rotate forwards to 44 (55 seconds) →→ select 44 twice →→ rotate forwards to 88 (44 seconds) →→ select 88. The total time taken is 2+5+4=112+5+4=11 seconds.
*/
